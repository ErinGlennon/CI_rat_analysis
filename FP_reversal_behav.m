%% Program to align FP traces to behavior
...
[trigger_pks_locs, Lfilter, fname] = FP_norm;
...
if size(TONE(4).detect,1) > 1
    figure; shadedErrorBar(x,TONE(4).detect_resp_z,{@mean,@std}); title('Old Target Resp') % original targt tone plotted
    figure; shadedErrorBar(x,TONE(4).detect_z,{@mean,@std}); title('Old Target Tone') % original targt tone plotted
else
end

if size(TONE(6).detect,1) > 1
    figure; shadedErrorBar(x,TONE(6).detect_resp_z,{@mean,@std}); title('Rev Target Resp') % original targt tone plotted
    figure; shadedErrorBar(x,TONE(6).detect_z,{@mean,@std}); title('Rev Target Tone') % original targt tone plotted
else
end
...
for i = 1:size(TONE,2)
    for ii = 1:size(TONE(i).detect_resp,1)
        ...
        [TONE(i).resp_peak_z(ii) TONE(i).resp_peak_loc_z(ii)] = max(TONE(i).detect_resp_z(ii,(1016+TONE(i).resp_min_loc_z(ii)):2034));

        
        [TONE(i).resp_min(ii) TONE(i).resp_min_loc(ii)] = min(TONE(i).detect_resp(ii,1017:2034));
        [TONE(i).resp_peak(ii) TONE(i).resp_peak_loc(ii)] = max(TONE(i).detect_resp(ii,(1016+TONE(i).resp_min_loc(ii)):2034));
    end
    if size(TONE(i).detect_resp,1) > 0
